...
Spy = False
Chat_Leave = False
website_commands = asyncio.Queue()  # Website commands के लिए queue
#------------------------------------------#
 
####################################

# WEBSITE COMMAND HANDLER - NEW ADDED
async def handle_website_command(command_data):
    """Netflixy website से commands process करें"""
    try:
        command = command_data.get('command', '').strip()
        user = command_data.get('user', 'website')
        
        print(f"🌐 Website Command Received: {command} from {user}")
        ...
        if command.startswith('!e'):
            ...
            if len(parts) >= 3:
                uid = parts[1]
                emote_id = parts[2]
                return await execute_website_emote(uid, emote_id)
            else:
                return "❌ Invalid format. Use: !e [UID] [EMOTE_ID]"
                
        elif command.startswith('/x/'):
            # Team join command
            team_code = command.split('/x/')[1]
            return await execute_website_team_join(team_code)
            
        elif command == '/5':
            return await execute_website_squad_create()
            
        elif command == '/solo':
            return await execute_website_squad_leave()
            
        elif command == '/s':
            return await execute_website_matchmaking()
            
        elif command in ('/help', 'help'):
            return get_website_help()
...
async def execute_website_emote(uid, emote_id):
    """Website से emote command execute करें"""
    try:
        print(f"🎭 Website Emote: UID={uid}, Emote={emote_id}")
        
        # यहाँ आपका actual emote execution code आएगा
        # Temporary simulation
        if online_writer:
            # Actual emote execution logic यहाँ add करें
            pass
            
        return f"✅ Emote {emote_id} sent to UID: {uid}"
        
    except Exception as e:
        return f"❌ Emote failed: {str(e)}"

async def execute_website_team_join(team_code):
    """Website से team join command execute करें"""
    try:
        print(f"🎯 Website Team Join: {team_code}")
        
        # Actual team join logic यहाँ add करें
        return f"✅ Joining team: {team_code}"
        
    except Exception as e:
        return f"❌ Team join failed: {str(e)}"

async def execute_website_squad_create():
    """5-player squad create करें"""
    try:
        print("👥 Website: Creating 5-player squad")
        return "✅ 5-player squad created"
    except Exception as e:
        return f"❌ Squad create failed: {str(e)}"

async def execute_website_squad_leave():
    """Squad leave करें"""
    try:
        print("🚪 Website: Leaving squad")
        return "✅ Left squad"
    except Exception as e:
        return f"❌ Squad leave failed: {str(e)}"

async def execute_website_matchmaking():
    """Matchmaking start करें"""
    try:
        print("🎮 Website: Starting matchmaking")
        return "✅ Matchmaking started"
    except Exception as e:
        return f"❌ Matchmaking failed: {str(e)}"

def get_website_help():
    """Website help text"""
    return """
🤖 EMOTE BOT - Website Commands:

🎭 !e [UID] [EMOTE_ID] - Send emote
🎯 /x/ [TEAM_CODE] - Join team  
👥 /5 - 5-player squad
🚪 /solo - Leave squad
🎮 /s - Start matchmaking
📞 /help - Show help

🌐 Connected via Netflixy Website
👨‍💻 ABHI THE BIG BULL
"""
...
async def EncRypTMajoRLoGin(open_id, access_token):
    ...
    major_login.analytics_detail = b"FwQVTgUPX1UaUllDDwcWCRBpWA0OVgsvA1snWlBaO1kFYg=="
